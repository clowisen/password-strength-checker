$(document).ready(function(){
    ...
    $('#password').on('input', function(){  // #password isimli input kutusunda bir değişiklik yapıldığı zaman bu fonksiyonun çalışmasını sağlar
        const password = $('#password').val(); // oluşturduğumuz password değişkenine #password inputundan gelen değeri atıyoruz
        if(password.match(/[a-z]/g)){ 
            checkpoints[0] = true  // eğer alfabedeki küçük harflerden herhangi birisi girilirse birinci şartı sağlıyor
        }
        else{
            checkpoints[0] = false // eğer sağlamıyorsa da false yapıyoruz ki input kutusundan değer silinirse barın uzunluğu gerilesin
        }
        if(password.match(/[A-Z]/g)){ 
        ...
        if(password.match(/[!,#,$,%,&,?]/g)){
            checkpoints[2] = true // eğer [...] içerisindeki işaretlerden herhangi biri girilirse üçüncü şart sağlanıyor
        }
        else{
            checkpoints[2] = false
        }
        if(password.match(/[0-9]/)){
            checkpoints[3] = true // eğer bir harf girilirse dördüncü şart da sağlanıyor
        }
        else{
            checkpoints[3] = false
        }
        ...
        switch (check) { // check değişkeninin değeri parolanın kaç tane şartı sağladığını gösterir 
            ...
            case 1:
                ...
                break; // bir şart sağlandıysa gereken ayarlamalar yapılıyor ve bu satırdan sonraki kodların okunmasına gerek kalınmadığı için break ile durduruluyor
            case 2:
